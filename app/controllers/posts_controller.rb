...
class PostsController < ApplicationController
  ...
  def index
    ...
    @all_ranks = Post.includes(:user).find(Like.group(:post_id).order(Arel.sql('count(post_id) desc')).limit(3).pluck(:post_id))
    @slider_posts = Post.includes(:user).where.not(image_id: nil).limit(8)
